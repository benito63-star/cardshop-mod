...
public class BoosterOpenScreen extends Screen {
    ...
    public void render(GuiGraphics g, int mx, int my, float delta) {
        ...
        int cx = width / 2;
        g.drawCenteredString(font, "Booster Pack Ouvert !",
            cx, 20, 0xFFFFFF);
        g.drawCenteredString(font,
            "Carte " + (current+1) + " / " + cards.size(),
            cx, 35, 0xAAAAAA);
        ...
        if (!cards.isEmpty()) {
            ...
            int cw = 120, ch = 160;
            int cx2 = cx - cw/2, cy = height/2 - ch/2;

            g.fill(cx2-2, cy-2, cx2+cw+2, cy+ch+2,
                0xFF000000 | rarity.color);
