...
public class BoosterResultPacket {
    ...
    public record CardInfo(String id, String rarityId, String name, long value) {}
    ...
    public static void encode(BoosterResultPacket pkt, FriendlyByteBuf buf) {
        ...
        for (CardInfo c : pkt.cards) {
            buf.writeUtf(c.id()); buf.writeUtf(c.rarityId());
            buf.writeUtf(c.name()); buf.writeLong(c.value());
        }
    }
    ...
    public static void handle(BoosterResultPacket pkt, PlayPayloadContext ctx) {
